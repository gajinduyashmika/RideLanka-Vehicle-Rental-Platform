...
export default function EditVehiclePage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const v: VehicleResponse = await vehiclesApi.getById(params.id as string);
        setForm({
          make: v.make,
          model: v.model,
          year: v.year,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await vehiclesApi.update(params.id as string, form);
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: '640px' }}>
      <h1 style={{ fontSize: '28px', fontWeight: 800, letterSpacing: '-0.02em', marginBottom: '8px' }}>
        Edit <span className="gradient-text">Vehicle</span>
      </h1>
